...
indirect enum AnyExpression: Hashable {
	enum UnaryOperator: String, Hashable {
		case not
	}

	enum BinaryOperator: String, Hashable {
		case and
		case equal
		case greaterThan
...
private extension AnyExpression.Function {
	var sql: SQL.Function {
		switch self {
		case .coalesce:
			return .coalesce
		case .count:
...
extension SQL.Expression {
	...
	private func dictionary(
		...
		return [column.name: [`operator`.rawValue: textValue]]
	}
}

public extension Expression where Model: PersistDB.Model {
	/// Create an expression from a keypath.
	init(_ keyPath: KeyPath<Model, Value>) {
		expression = AnyExpression(keyPath)
	}
}

public extension Expression where Model == None, Value == Date {
	/// An expression that evaluates to the current datetime.
	static var now: Expression {
		return Expression(AnyExpression.now)
	}
}

public extension Expression where Model == None, Value: ModelValue {
	init(_ value: Value) {
		expression = .value(Value.anyValue.encode(value).sql)
	}
}

public extension Expression where Model == None, Value: OptionalProtocol, Value.Wrapped: ModelValue {
	init(_ value: Value?) {
		expression = .value(value.map(Value.Wrapped.anyValue.encode)?.sql ?? .null)
	}
}

public extension Expression where Value == String {
	/// The number of characters in the string prior to the first null character.
	var count: Expression<Model, Int> {
...
func == (lhs: AnyExpression, rhs: AnyExpression) -> AnyExpression {
...
prefix func ! (expression: AnyExpression) -> AnyExpression {
...
func max(_ expressions: [AnyExpression]) -> AnyExpression {
...
func min(_ expressions: [AnyExpression]) -> AnyExpression {
...
extension Collection where Iterator.Element: ModelValue {
	...
	/// expression.
	func contains(_ expression: AnyExpression) -> AnyExpression {
